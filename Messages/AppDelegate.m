  ...
  if ([NSThread isMainThread]) {
    ...
    NSManagedObjectContext *context =
        [[[NSThread currentThread] threadDictionary]
            objectForKey:@"managedObjectContext"];
...
- (NSManagedObjectModel *)managedObjectModel {
  ...
  NSURL *modelURL =
      [[NSBundle mainBundle] URLForResource:@"DataModel" withExtension:@"momd"];
  _managedObjectModel =
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
  ...
  if (![fileManager fileExistsAtPath:storeURL.path]) {
    NSString *defaultStorePath =
        [[NSBundle mainBundle] pathForResource:@"DataModel" ofType:@"sqlite"];
    if (defaultStorePath) {
...
- (TCMetaPost *)fetchMetaPostForEntity:(NSString *)entity
  ...
  NSFetchRequest *fetchRequest =
      [[NSFetchRequest alloc] initWithEntityName:@"TCMetaPost"];
  ...
  if ([appPostManagedObject isKindOfClass:TCAppPostManagedObject.class]) {
    metaPost = [MTLManagedObjectAdapter modelOfClass:TCMetaPost.class
                                   fromManagedObject:appPostManagedObject
                                               error:error];
  } else {
    if (error) {
      *error = [[NSError alloc] initWithDomain:@"Meta post not found"
                                          code:1
                                      userInfo:nil];
    }
  }
...
- (TCAppPost *)currentAppPost {
  if (!currentAppPostObjectID)
    return nil;

  NSManagedObject *appPostManagedObject =
      [[self mainManagedObjectContext] objectWithID:currentAppPostObjectID];

  if (!appPostManagedObject)
    return nil;

  return (TCAppPost *)[MTLManagedObjectAdapter modelOfClass:TCAppPost.class
                                          fromManagedObject:appPostManagedObject
                                                      error:nil];
}

- (void)setCurrentAppPost:(TCAppPost *)appPost {
  NSManagedObject *appPostManagedObject = [MTLManagedObjectAdapter
      managedObjectFromModel:appPost

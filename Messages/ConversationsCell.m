...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  refreshTimer =
      [NSTimer scheduledTimerWithTimeInterval:1
                                       target:self
                                     selector:@selector(refreshTimeViewContent)
                                     userInfo:nil
                                      repeats:YES];
...
- (void)layoutSubviews {
  ...
  if (self.editing) {
    ...
    ConversationsViewController *c =
        (ConversationsViewController *)self.tableViewController;
  ...
  CGRect contentFrame =
      CGRectMake(imageFrame.origin.x + imageFrame.size.width + 5, // x
                 imageFrame.origin.y,                             // y
                 self.frame.size.width - imageFrame.origin.x -
                     imageFrame.size.width - 13, // width
  ...
  // name view frame
  nameView.frame = CGRectMake(
      0, 0, contentFrame.size.width - timeView.frame.size.width - 5, 20);
  ...
  // body view frame
  bodyView.frame =
      CGRectMake(0, nameView.frame.origin.y + nameView.frame.size.height,
                 contentFrame.size.width,
                 contentFrame.size.height - nameView.frame.origin.y -
                     nameView.frame.size.height - 5);
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated {
  ...
  self.tableViewController.tableView.allowsMultipleSelectionDuringEditing =
      editing;
...
- (void)initConversation:(Conversation *)conversation {
  ...
  contacts = [self.conversation.contacts
...
- (void)refreshTimeViewContent {
  ...
  if (self.conversation.latestMessage) {
    view.text =
        [self.conversation.latestMessage.timestamp dateTimeAgo]; // TODO: update
                                                                 // in view
                                                                 // every n
                                                                 // seconds
  } else {
...
- (void)refreshNameViewContent {
  ...
  [MessagesAesthetics setFontForLabel:view
...
- (void)refreshImageViewContent {
  ...
  for (int i = 0; i < displayCount; i++) {
    ...
    avatar = [[UIImage imageWithData:contact.avatar]
              thumbnailImage:avatarSize
           transparentBorder:0
                cornerRadius:3
  ...
  if (nMore > 0) {
    ...
    UIView *nMoreView = [[UIView alloc]
        initWithFrame:CGRectMake(offsetX, offsetY, avatarSize, avatarSize)];
    ...
    UILabel *nMoreText = [[UILabel alloc]
        initWithFrame:CGRectMake(1.5f, 1.5f, avatarSize - 3, avatarSize - 3)];

...
- (void)layoutSubviews {
  ...
  messageBubbleView.frame =
      CGRectMake(messageBubbleView.frame.origin.x,   // x
  ...
  self.frame = CGRectMake(self.frame.origin.x,   // x
                          ...
                          messageBubbleView.frame.size.height +
                              messageBubbleView.frame.origin.y // height
                          );
...
- (void)refreshStatusViewContent {
  ...
  UILabel *textView = [[UILabel alloc]
      initWithFrame:CGRectMake(0, 0, view.frame.size.width - 10, 0)];
  textView.text = [self getStringForMessageState];
  textView.textColor = [MessagesAesthetics greyColor];
  textView.numberOfLines = 0;
  [MessagesAesthetics setFontForLabel:textView
                            withStyle:UIFontTextStyleCaption2];
  [textView sizeToFit];
...
- (CGRect)getBubbleFrameForSide {
  ...
  if (alignment == ConversationMessageRight) {
    width -= 18;
    frame = CGRectMake(self.frame.size.width - width - offsetX, offsetY, width,
                       height);
  } else {
...
- (CGRect)getBubbleNibFrameForFrame:(CGRect)frame {
  ...
  if (alignment == ConversationMessageLeft) {
    return CGRectMake(frame.origin.x - nibWidth, frame.origin.y + 15, nibWidth,
                      nibHeight);

  } else {
    return CGRectMake(frame.origin.x + frame.size.width, frame.origin.y + 15,
                      nibWidth, nibHeight);
  }
}

...
- (instancetype)init {
  ...
  NSString *plistPath =
      [rootPath stringByAppendingPathComponent:@"Cursors.plist"];

  // Look in application Resources if not in Documents
  if (![[NSFileManager defaultManager] fileExistsAtPath:plistPath]) {
    plistPath =
        [[NSBundle mainBundle] pathForResource:@"Cursors" ofType:@"plist"];
  }
  ...
  if (!relationshipCursor) {
    relationshipCursor =
        @{ @"version" : @"", @"timestamp" : [NSNumber numberWithInteger:0] };
  }

  self.relationshipCursorTimestamp =
      [self deserializeTimestamp:(NSNumber *)
            [relationshipCursor objectForKey:@"timestamp"]];

  self.relationshipCursorVersionID =
      (NSString *)[relationshipCursor objectForKey:@"version"];
  ...
  self.messageCursorTimestamp =
      [self deserializeTimestamp:(NSNumber *)
            [messageCursor objectForKey:@"timestamp"]];

  self.messageCursorVersionID =
      (NSString *)[messageCursor objectForKey:@"version"];
...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"<%@ <Relationship timestamp=%f version=%@>>", self.class,
          [self.relationshipCursorTimestamp timeIntervalSince1970] * 1000,
          self.relationshipCursorVersionID];
}
...
- (BOOL)saveToPlistWithError:(NSError *__autoreleasing *)error {
  ...
  NSDictionary *plistDictionary = [NSDictionary
      dictionaryWithObjects:@[ relationshipCursor, messageCursor ]
                    forKeys:@[ @"RelationshipCursor", @"MessageCursor" ]];
  ...
  NSData *plistXML = [NSPropertyListSerialization
      dataFromPropertyList:plistDictionary
...
- (NSString *)stringFromTimestamp:(NSDate *)timestamp
  ...
  NSString *res =
      [formatter stringFromNumber:[self serializeTimestamp:timestamp]];
...
- (NSDate *)deserializeTimestamp:(NSNumber *)timestamp {
  return [NSDate dateWithTimeIntervalSince1970:
                     [[NSNumber numberWithDouble:([timestamp floatValue] /
                                                  1000)] integerValue]];
}

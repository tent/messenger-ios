...
- (void)viewDidLoad {
  ...
  UIGestureRecognizer *tapParent = [[UITapGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(handleTableTap:)];
  ...
  ConversationTitleView *titleView =
      [[ConversationTitleView alloc] initWithFrame:frame withDataSource:self];
  [self.navigationItem setTitleView:titleView];

  UIButton *participantsButton =
      [[UIButton alloc] initWithFrame:titleView.frame];
  [titleView addSubview:participantsButton];
  [titleView bringSubviewToFront:participantsButton];
  [participantsButton addTarget:self
                         action:@selector(participantsButtonPressed:)
...
- (void)viewDidAppear:(__unused BOOL)animated {
  ...
  for (UIViewController *c in self.navigationController.viewControllers) {
    if (![[NSString stringWithFormat:@"%@", c.class]
             isEqual:@"ContactsViewController"]) {
...
- (void)keyboardWillShow:(NSNotification *)notification {
  ...
  // Because the "space" is actually the difference between the bottom lines of
  // the 2 views,
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(__unused id)sender {
  if ([segue.identifier isEqualToString:@"loadParticipants"]) {
    ParticipantsViewController *participantsViewController =
        (ParticipantsViewController *)([segue destinationViewController]);
...
- (NSInteger)tableView:(__unused UITableView *)tableView
  ...
  id<NSFetchedResultsSectionInfo> sectionInfo =
      [[fetchedResultsController sections] objectAtIndex:(unsigned int)section];
...
- (CGFloat)tableView:(__unused UITableView *)tableView
  ...
  /*
   This is called for every row in the table before any of the cells are
   rendered
...
- (BOOL)tableView:(__unused UITableView *)tableView
    shouldShowMenuForRowAtIndexPath:(__unused NSIndexPath *)indexPath {
  return YES;
}

- (BOOL)tableView:(__unused UITableView *)tableView
     canPerformAction:(SEL)action
    forRowAtIndexPath:(__unused NSIndexPath *)indexPath
...
- (void)sendButtonPressed:(__unused id)sender {
  ...
  NSRegularExpression *emptyStringRegex = [NSRegularExpression
  ...
  message.conversation.latestMessage =
      message; // TODO: use an observer for this
  message.timestamp = [[NSDate alloc] init];
  message.state =
      [NSNumber numberWithUnsignedInteger:ConversationMessageDelivering];
...
- (NSFetchedResultsController *)fetchedResultsController {
  ...
  NSPredicate *predicate = [NSPredicate
      predicateWithFormat:@"conversation == %@", self.conversation];
...
- (void)scrollToBottom {
  if (self.tableView.contentSize.height > self.tableView.bounds.size.height) {
    [self.tableView
        setContentOffset:CGPointMake(0, self.tableView.contentSize.height -

...
+ (void)persistObjectID:(NSManagedObjectID *)objectID {
  ...
  if (!appPost || !appPost.authCredentialsPost)
    return;
  ...
  client.credentialsPost = appPost.authCredentialsPost;
    ...
    NSMutableArray *conversationEntities = [NSMutableArray arrayWithArray:[[conversation.contacts allObjects] transposedArrayUsingBlock:^id(Contact *contact) {
      return [[contact.relationshipPost.mentions objectAtIndex:0]
          valueForKey:@"entity"];
    }]];
...
+ (void)createMessagesWithConversationEntities:(NSArray *)conversationEntities
    ...
    [conversation.messages enumerateObjectsUsingBlock:^(Message *message, __unused BOOL *stop) {
      ...
      messagePost.content = @{ @"text" : message.body };
...
+ (void)fetchNewMessages {
  ...
  // since cursor
  [feedParams
      addValue:[cursors stringFromTimestamp:cursors.messageCursorTimestamp
                                    version:cursors.messageCursorVersionID]
        forKey:@"since"];

  __block BOOL fetchMessagesComplete = NO;

  void (^completionBlock)() = ^{ fetchMessagesComplete = YES; };
    ...
    [self fetchNewMessagesWithClient:client params:feedParams successBlock:^(__unused AFHTTPRequestOperation *operation, TCResponseEnvelope *responseEnvelope) {
      ...
      NSMutableDictionary *conversationManagedObjects =
          [[NSMutableDictionary alloc] init];

        [[responseEnvelope refs] enumerateObjectsUsingBlock:^(TCPost *postModel, __unused NSUInteger idx, __unused BOOL *stop) {
          if (![postModel.typeURI
                   hasPrefix:@"https://tent.io/types/conversation/v0"])
            return;

          // Check if we already have this conversation the in db
          NSError *fetchConversationError;
          Conversation *existingConversation =
              [self conversationForPostID:postModel.ID
                                    error:&fetchConversationError];

          if (existingConversation) {
            [conversationManagedObjects setObject:existingConversation
                                           forKey:postModel.ID];
          } else {
            if (fetchConversationError) {
              NSLog(@"Error fetching existing conversation: %@",
                    fetchConversationError);
            }

            NSError *error;

            TCPostManagedObject *conversationPostManagedObject =
                [MTLManagedObjectAdapter managedObjectFromModel:postModel
                                           insertingIntoContext:context
                                                          error:&error];

            if (error) {
              NSLog(
                  @"Error transposing conversation post into manged object: %@",
                  error);
              return;
            }

            Conversation *conversationManagedObject = [[Conversation alloc]
                                initWithEntity:
                                    [NSEntityDescription
                                                 entityForName:@"Conversation"
                ...
                [postModel.mentions enumerateObjectsUsingBlock:^(NSDictionary *mention, __unused NSUInteger _idx, __unused BOOL *_stop) {
                  if ([mention valueForKey:@"post"])
                    return; // Skip any mentioned posts
                  if (![mention valueForKey:@"entity"])
                    return; // Skip mentions referencing conversation post
                            // entity
                  if ([[mention valueForKey:@"entity"]
                          isEqualToString:[appPost.entityURI absoluteString]])
                    return; // Skip mentions of self

                  NSError *contactLookupError;
                  Contact *contact =
                      [self contactForEntity:[mention valueForKey:@"entity"]
                                       error:&contactLookupError];

                  if (contact) {
                    [conversationManagedObject addContactsObject:contact];
                  } else {
                    NSLog(@"Error looking up Contact for <Entity %@>: %@",
                          [mention valueForKey:@"entity"], error);
                  }
                }];

                if (![[postModel.entityURI absoluteString]
                         isEqualToString:[appPost.entityURI absoluteString]]) {
                  [conversationManagedObject
                      addContactsObject:
                          [self contactForEntity:
                                  [postModel.entityURI absoluteString]
                                           error:&error]];
                }

                conversationManagedObject.conversationPost =
                    conversationPostManagedObject;

                [conversationManagedObjects setObject:conversationManagedObject
                                               forKey:postModel.ID];
          }
        }];
        ...
        [[responseEnvelope posts] enumerateObjectsUsingBlock:^(TCPost *postModel, __unused NSUInteger idx, __unused BOOL *stop) {
          ...
          Message *existingMessage =
              [self messageForPostID:postModel.ID error:&fetchMessageError];
          ...
          NSString *conversationPostID = [[postModel.refs objectAtIndex:0]
              valueForKey:@"post"]; // TODO: find conversation ref as it's not
                                    // garenteed to be the first
          Conversation *conversationManagedObject =
              [conversationManagedObjects valueForKey:conversationPostID];

          NSError *error;

          TCPostManagedObject *messagePostManagedObject =
              [MTLManagedObjectAdapter managedObjectFromModel:postModel
                                         insertingIntoContext:context
                                                        error:&error];

          Message *messageManagedObject =
              [[Message alloc] initWithEntity:
                                   [NSEntityDescription entityForName:@"Message"
                                               inManagedObjectContext:context]
                  insertIntoManagedObjectContext:context];

          // TODO: Configure entity/managed object for message post
          messageManagedObject.body =
          ...
          if (!conversationManagedObject.latestMessage ||
              ([messageManagedObject.timestamp timeIntervalSince1970] >
               [conversationManagedObject.latestMessage
...
failureBlock:
  ^(AFHTTPRequestOperation * operation, NSError * error) {
    ...
    NSLog(@"failed to fetch message posts: %@ with request: %@ response: %@",
          error, [[NSString alloc] initWithData:[operation.request HTTPBody]
...
+ (void)fetchNewMessagesWithClient:(TentClient *)client
                      ...
                      failureBlock:
                          (void (^)(AFHTTPRequestOperation *, NSError *))failure
    ...
    [client postsFeedWithParams:feedParams successBlock:^(AFHTTPRequestOperation *operation, TCResponseEnvelope *responseEnvelope) {
      ...
      if ([responseEnvelope isEmpty]) {
        ...
        [self fetchNewMessagesWithClient:client
...
+ (Contact *)contactForEntity:(NSString *)entity
...
+ (Conversation *)conversationForPostID:(NSString *)postID
  ...
  NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc]
      initWithKey:@"conversationPost.versionPublishedAt"
        ascending:NO];
  [fetchRequest setSortDescriptors:@[ sortDescriptor ]];

  NSPredicate *predicate =
      [NSPredicate predicateWithFormat:@"conversationPost.id == %@", postID];
  [fetchRequest setPredicate:predicate];
...
+ (Message *)messageForPostID:(NSString *)postID
  ...
  NSFetchRequest *fetchRequest =
      [[NSFetchRequest alloc] initWithEntityName:@"Message"];

  // Configure sort order
  NSSortDescriptor *sortDescriptor =
      [[NSSortDescriptor alloc] initWithKey:@"messagePost.versionPublishedAt"
                                  ascending:NO];
  [fetchRequest setSortDescriptors:@[ sortDescriptor ]];

  NSPredicate *predicate =
      [NSPredicate predicateWithFormat:@"messagePost.id == %@", postID];
  [fetchRequest setPredicate:predicate];
...
- (void)prepareForDeletion {
   ...
   * All messages within this conversation will be deleted via the delete rule
   * in Core Data
   */
...
failureBlock:
  ^(__unused AFHTTPRequestOperation * operation, NSError * error) {
    NSLog(@"Error deleting conversation post: %@ via <%@ %@>", error,
